...
To install the neovim dotfiles. Run the following command from inside the `files/` directory in this repository:
```bash
stow neovim -t "$HOME"
```

To install the VSCode dotfiles. Run the following command inside the `files/` directory in this repository:
```bash
stow vscode -t "$HOME"
```
